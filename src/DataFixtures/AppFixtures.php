...
class AppFixtures extends Fixture
{
  ...
  {
    for ($i = 1; $i < $this->faker->numberBetween(50, 70); $i++) {
      $address = new Address();
  ...
  public function loadUsers(ObjectManager $manager)
  {
    ...
    for ($i = 1; $i < $this->faker->numberBetween(40, 80); $i++) {
      $user = new User();
  ...
  public function loadCourses(ObjectManager $manager)
  {
    ...
    $coursesNumber = $this->faker->numberBetween($per_page * 3, $per_page * 8);
    ...
    for ($i = 1; $i <= ceil($coursesNumber / $per_page); $i++) {
      ...
      $pexelsApiResponse = file_get_contents("https://api.pexels.com/v1/videos/search?query=code&per_page=$per_page&page=$i", false, $context);
      $json = json_decode($pexelsApiResponse, true);
